...
public slots:
    ...
    void SendCustomerInfo(ClientItem* c);               /*Clientmanager에서 받아온 고객 정보 중 필요한 데이터만 뽑아서 트리위젯에 추가하는 슬롯*/
    void SendProductInfo(ProductItem* c);               /*Productmanager에서 받아온 상품 정보 중 필요한 데이터만 뽑아서 트리위젯에 추가하는 슬롯*/


private slots:
    /* QTreeWidget을 위한 슬롯 */
    void on_treeWidget_itemClicked(QTreeWidgetItem *item, int column);  /*트리위젯의 주문내역 정보 클릭시 동작하는 함수*/
