...
void ShoplistManagerForm::loadData()
{
    ...
    if(db.open()){
        ...
        shopModel->setHeaderData(2, Qt::Horizontal, tr("Customer Info"));
        shopModel->setHeaderData(3, Qt::Horizontal, tr("Product Info"));
        shopModel->setHeaderData(4, Qt::Horizontal, tr("Quantity"));
...
void ShoplistManagerForm::on_searchPushButton_clicked()
{
    ...
    foreach(auto ix, indexes) {
        ...
        strings << QString::number(id) << Date << CustomerInfo << ProductInfo
                << QString::number(Quantity) << Address
                << QString::number(Price) << QString::number(TotalPrice);
